...
readme = re.sub(
    r' \[(.*)\]\((?!http)(.*)\)', r' [\1](https://github.com/%s/%s/blob/master/\2)' % (github_user, github_repo), readme
)
...
PACKAGES = [bpdl.__name__, 'experiments']
...
def run_apidoc(_):
    for pkg in PACKAGES:
        argv = [
            ...
            os.path.join(PATH_ROOT, 'experiments', 'test_*'), '--force'
        ]
        try:
            # Sphinx 1.7+
